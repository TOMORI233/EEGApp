...
1. 安装**`MATLAB 2023b`**及以上版本，以及在附加功能中安装以下工具包：

> Signal processing toolbox

2. 本项目使用需要`MATLAB-Utils`项目，Git地址`git@github.com:Yulab-ZJU/MATLAB-Utils.git`
...
% 将pID=101的rules强制拼接至原rules.xlsx表格下（新增原来不存在的列）
...
8. 完成一个protocol的记录，自动保存数据为`[pID].mat`，手动/自动进入下一阶段。保存的内容包括：trial数据（trialIndex, events, 单位：sec）、rules。同时在该目录下也会保存一个`config.mat`
9. 自动/手动（强制）结束记录
...
- 2026/01/11 by XHX - **Incompatible**
  ...
  | Type   | Target                        | Content                                                      |
  | ------ | ----------------------------- | ------------------------------------------------------------ |
  | Modify | `rulesGenerator.m`            | 重构了需要的参数，删除了`ITI`, `apType`, `cueLag`等属性，新增`eventFlow`用于控制事件流，包括原`ITI`和`cueLag`的功能 |
  | Add    | `eventFlowApp.mlapp`          | 用于生成事件流table                                          |
  | Delete | `activeFcn.m`, `passiveFcn.m` | 用新的`presentStimuli.m`替代                                 |
  | Add    | `presentStimuli.m`            | 新的刺激呈现函数，支持多刺激、跨模态(听觉/视觉)呈现          |
